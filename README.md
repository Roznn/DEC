...
The model run by default on `MNISToriginal`  is $\mathcal{M}_{0.9}^{16}$  and accuracy score first reported is for $\mathcal{I}_{\mathbf{y}}^{\mathbf{x}}$ (acc=1), the second is for $\mathcal{I}_{\mathbf{0}}^{\mathbf{x}}$ (acc.= 0.80798) and the last is for test set $\mathcal{I}_{\mathbf{0}}^{\mathbf{x}'}$ (acc.=0.80930)
