...
Our approach uses PCA trained on a dataset with samples $\mathbf{z}_{\alpha}^{(i)}$ $=$ $\lbrack (1-\alpha)\mathbf{x}^{(i)} \quad \alpha\mathbf{y}^{(i)}\rbrack^T$. Principal Components are then used for classification even if no class information is available at test time to process a new input $\mathbf{x}$.
