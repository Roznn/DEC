...
Our approach uses PCA trained on a dataset with samples $\mathbf{z}_{\alpha}^{(i)}$ combines in one vector  sample $( (1-\alpha)\mathbf{x}^{(i)},\alpha\mathbf{y}^{(i)})$ from $\mathcal{B}$. Principal Component are then used for classification even if no class information is available at test time for a new input $\mathbf{x}$.
